...
ShellRoot {
    ...
    FileView {
        id: favoritesFile
        path: "/home/arise/.config/serafin/favorites.json"
        watchChanges: true
        onLoaded: favoritesManager.load(favoritesFile.text())
    ...
    QtObject {
        ...
        function save() {
            ...
            saveFavProc.command = ["sh", "-c", "echo '" + content + "' > /home/arise/.config/serafin/favorites.json"];
        ...
        function toggle(app) {
            ...
            for (var i = 0; i < list.length; i++) {
                if (list[i].exec === app.exec) { idx = i; break; }
            }
            if (idx >= 0) {
                list.splice(idx, 1);
            } else {
        ...
        function isFavorite(app) {
            for (var i = 0; i < list.length; i++) {
                if (list[i].exec === app.exec) return true;
            }
            return false;
        }

        function updateHaloSubs() {
            var subs = [];
            var max = Math.min(6, list.length);
            var colors = ["#ff8c5a", "#3e9bff", "#b4befe", "#f9e2af", "#1ed760", "#cba6f7"];
            for (var i = 0; i < max; i++) {
...
"), devs = [];
    ...
    QtObject {
        ...
        property bool haloOpen: false        // el menú radial (halo) está abierto
        ...
        readonly property bool hasWindows: {
            ...
            return false;
               ...
               readonly property bool revealed:    pinned || !hasWindows || atBottom || overBar || anyPopoutOpen || wallOpen || haloOpen
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    RowLayout {
                        ...
                        Text { text: "\uf002"; color: theme.accent; font.family: "Symbols Nerd Font"; font.pixelSize: 13 }
                        Text { id: searchPlaceholder; text: "Buscar aplicaciones…"; color: theme.textDim; font.family: "Cantarell"; font.pixelSize: 13; Layout.fillWidth: true; elide: Text.ElideRight }
                    }
                    MouseArea {
                        anchors.fill: parent
                        cursorShape: Qt.PointingHandCursor
                        onClicked: {
                            appLauncherOverlay.openWithSearch("");
                        }
                    }
                }
    ...
    Connections {
        ...
        function onShownChanged() {
            ui.wallOpen = wallpaperPicker.shown;
            if (wallpaperPicker.shown && halo.shown) halo.close();
        }
    }
    ...
    // ══════════════════════════════════════════════════════════
    //  HALO · menú radial (capa Top, debajo de barra y pulse)
    // ══════════════════════════════════════════════════════════
    Halo {
        id: halo
        appFavorites: favoritesManager.haloSubs
        accent:  theme.accent
        accent2: theme.accent2
        barBg:   theme.barBg
        textCol: theme.text
        onRequestWallpaper: wallpaperPicker.open()
        onRequestWallpaperRandom: wallRandomProc.running = true
    }
    Process { id: wallRandomProc; command: ["/home/arise/.local/bin/serafin-wall", "--random"] }
    Connections {
        target: halo
        function onShownChanged() {
            ui.haloOpen = halo.shown;
            if (halo.shown) {
                if (wallpaperPicker.shown) wallpaperPicker.close();
            } else {
                appLauncherOverlay.close();
            }
        }
    }
    IpcHandler {
        target: "halo"
        function toggle(): void { halo.toggle(); }
        function open(): void  { halo.open(); }
        function close(): void { halo.close(); }
    }

    // ══════════════════════════════════════════════════════════
    //  APP LAUNCHER OVERLAY · panel estilo caelestia
    // ══════════════════════════════════════════════════════════
    Process {
        id: appListProc
        command: ["sh", "-c", "grep -rlL 'NoDisplay=true' /usr/share/applications/*.desktop /home/arise/.local/share/applications/*.desktop 2>/dev/null | while read f; do name=$(grep '^Name=' \"$f\" | head -1 | cut -d= -f2); exec=$(grep '^Exec=' \"$f\" | head -1 | cut -d= -f2 | sed 's/ %[fFuUdDnNickvm]//g'); icon=$(grep '^Icon=' \"$f\" | head -1 | cut -d= -f2); [ -n \"$name\" ] && [ -n \"$exec\" ] && echo \"$name|$exec|$icon\"; done | sort -t'|' -k1 -fu"]
        stdout: StdioCollector {
            ...
            onStreamFinished: {
                ...
                for (var i = 0; i < lines.length; i++) {
                    var p = lines[i].split("|");
                    if (p.length >= 2) result.push({name: p[0], exec: p[1], icon: p.length >= 3 ? p[2] : ""});
                }
                appLauncherOverlay.allApps = result;
            }
        }
    }
    Timer { interval: 100; running: true; repeat: false; onTriggered: appListProc.running = true }

    Process { id: appExecProc }
    ...
    PanelWindow {
        ...
        function openWithSearch(q) {
            query = q || "";
            if (menuSearchInput.text !== query) menuSearchInput.text = query;
            filterApps();
            if (!isOpen) { isOpen = true; launcherOpenAnim.restart(); }
            menuSearchInput.forceActiveFocus();
        }
        function close() {
            if (isOpen) { launcherCloseAnim.restart(); }
        }
        function filterApps() {
            if (!query || query.trim().length < 2) { filtered = []; return; }
            var q = query.toLowerCase().trim();
            var res = [];
            for (var i = 0; i < allApps.length && res.length < 24; i++) {
                if (allApps[i].name.toLowerCase().indexOf(q) >= 0) res.push(allApps[i]);
            }
            filtered = res;
        }
        ...
        exclusiveZone: 0
        WlrLayershell.layer: WlrLayer.Overlay
        WlrLayershell.keyboardFocus: appLauncherOverlay.isOpen ? WlrKeyboardFocus.Exclusive : WlrKeyboardFocus.None
        mask: Region { item: appLauncherOverlay.isOpen && appLauncherOverlay.openProgress > 0.01 ? launcherPanel : null }
        visible: appLauncherOverlay.openProgress > 0.001

        NumberAnimation {
            id: launcherOpenAnim; target: appLauncherOverlay; property: "openProgress"
            from: 0; to: 1; duration: 420; easing.type: Easing.OutBack; easing.overshoot: 1.15
        }
        NumberAnimation {
            id: launcherCloseAnim; target: appLauncherOverlay; property: "openProgress"
            to: 0; duration: 220; easing.type: Easing.InCubic
            onFinished: { appLauncherOverlay.isOpen = false; appLauncherOverlay.query = ""; appLauncherOverlay.filtered = []; }
        }

        // Click fuera cierra
        MouseArea {
            anchors.fill: parent
            visible: appLauncherOverlay.isOpen
            onClicked: {
                appLauncherOverlay.close();
            }
        }

        // Contenedor del panel con forma orgánica estilo Caelestia
        Item {
            id: launcherPanel
            width: Math.min(620, parent.width * 0.48)
            height: launcherCol.implicitHeight + 36

            anchors.horizontalCenter: parent.horizontalCenter
            anchors.top: parent.top
            anchors.topMargin: 0

            // Canvas vectorial para la forma de masa estirada Caelestia
            Canvas {
                id: shapeCanvas
                enabled: false
                anchors.fill: parent
                onPaint: {
                    ...
                    var r = 24; // radio esquinas inferiores
                    var cr = 28; // radio de la curva cóncava suave hacia los bordes superiores

                    ctx.save();
                    ctx.beginPath();
                    
                    // Empezamos arriba a la izquierda (borde de la pantalla)
                    ctx.moveTo(0, 0);
                    
                    // Curva suave hacia la izquierda que conecta el panel con el borde superior de la pantalla
                    ctx.quadraticCurveTo(cr, 0, cr, 12);
                    
                    // Lado izquierdo bajando
                    ctx.lineTo(cr, h - r);
                    
                    // Esquina inferior izquierda redondeada
                    ctx.arcTo(cr, h, cr + r, h, r);
                    
                    // Borde inferior hacia la derecha
                    ctx.lineTo(w - cr - r, h);
                    
                    // Esquina inferior derecha redondeada
                    ctx.arcTo(w - cr, h, w - cr, h - r, r);
                    
                    // Lado derecho subiendo
                    ctx.lineTo(w - cr, 12);
                    
                    // Curva suave hacia la derecha que se funde con el borde de la pantalla
                    ctx.quadraticCurveTo(w - cr, 0, w, 0);
                    
                    ctx.closePath();

                    // Relleno oscuro semi-transparente estilo glass
                    ctx.fillStyle = "rgba(14, 14, 18, 0.94)";
                    ctx.fill();

                    // Borde sutil solo en los costados e inferior (sin borde arriba)
                    ctx.strokeStyle = "rgba(" + (theme.accent.r * 255) + "," + (theme.accent.g * 255) + "," + (theme.accent.b * 255) + ", 0.35)";
                    ctx.lineWidth = 1.5;
                    ctx.stroke();
                ...
                Connections {
                    target: theme
                    function onAccentChanged() { shapeCanvas.requestPaint(); }
                }
            ...
            // Animación caelestia: estirado elástico desde el borde superior
            transform: Scale {
                origin.x: launcherPanel.width / 2
                origin.y: 0
                xScale: 0.82 + 0.18 * appLauncherOverlay.openProgress
                yScale: appLauncherOverlay.openProgress
            }
            opacity: Math.min(1, appLauncherOverlay.openProgress * 2.5)
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    TextInput {
                        ...
                        font.pixelSize: 15
                        font.bold: true
                        focus: appLauncherOverlay.isOpen
                        clip: true
                        Text {
                            text: "Escribe para buscar aplicaciones…"
                            color: theme.textDim
                            font.family: "Cantarell"
                            font.pixelSize: 15
                        ...
                        onTextChanged: {
                            appLauncherOverlay.query = text;
                            appLauncherOverlay.filterApps();
                        }
                    ...
                    Text {
                        text: appLauncherOverlay.filtered.length + " apps"
                        color: theme.textDim
                        font.family: "Cantarell"
                        font.pixelSize: 11
                    }
                ...
                Rectangle { 
                    ...
                    gradient: Gradient {
                        ...
                        GradientStop { position: 0.5; color: Qt.rgba(theme.accent.r, theme.accent.g, theme.accent.b, 0.4) }
                ...
                // Favoritos Headers
                ColumnLayout {
                    visible: appLauncherOverlay.query.trim().length === 0 && favoritesManager.list.length > 0
                    Layout.fillWidth: true
                    spacing: 8
                    ...
                    RowLayout {
                        ...
                        spacing: 8
                        Repeater {
                            model: favoritesManager.list
                            delegate: Rectangle {
                                required property var modelData
                                required property int index
                                width: (launcherPanel.width - 32 - 24) / 5
                                height: 80
                                radius: 14
                                color: favMa.containsMouse ? Qt.rgba(theme.accent.r, theme.accent.g, theme.accent.b, 0.15) : Qt.rgba(1,1,1,0.04)
                                Behavior on color { ColorAnimation { duration: 140 } }
                                
                                transform: Scale {
                                    origin.x: width / 2; origin.y: height / 2
                                    xScale: favMa.containsMouse ? 1.05 : 1.0
                                    yScale: favMa.containsMouse ? 1.05 : 1.0
                                ...
                                ColumnLayout {
                                    ...
                                    Text {
                                        Layout.alignment: Qt.AlignHCenter
                                        text: modelData.icon || "\uf009"
                                        color: favMa.containsMouse ? theme.accent : theme.text
                                        font.family: "Symbols Nerd Font"
                                        font.pixelSize: 24
                                        Behavior on color { ColorAnimation { duration: 140 } }
                                    }
                                ...
                                Rectangle {
                                    ...
                                    Text {
                                        ...
                                        font.pixelSize: 10
                                        layer.enabled: true
                                        layer.effect: MultiEffect { shadowEnabled: true; shadowColor: theme.accent; shadowBlur: 0.5 }
                                    }
                                    MouseArea {
                                        anchors.fill: parent
                                        cursorShape: Qt.PointingHandCursor
                                        onClicked: favoritesManager.toggle(modelData)
                                    }
                                }
                                ...
                                MouseArea {
                                    ...
                                    cursorShape: Qt.PointingHandCursor
                                    onClicked: {
                                        appExecProc.command = ["sh", "-c", modelData.exec];
                                        appExecProc.running = true;
                                        appLauncherOverlay.close();
                                    }
                                }
                            }
                    ...
                    Rectangle { Layout.fillWidth: true; height: 1; color: Qt.rgba(1,1,1,0.06); Layout.topMargin: 8; Layout.bottomMargin: 8 }
                }

                // Grid de apps
                Grid {
                    visible: appLauncherOverlay.filtered.length > 0
                    Layout.fillWidth: true
                    columns: 5
                    spacing: 6
                    Repeater {
                        model: appLauncherOverlay.filtered
                        delegate: Rectangle {
                            required property var modelData
                            required property int index
                            width: (launcherPanel.width - 32 - 24) / 5
                            height: 80
                            radius: 14
                            color: appMa.containsMouse ? Qt.rgba(theme.accent.r, theme.accent.g, theme.accent.b, 0.15) : Qt.rgba(1,1,1,0.04)
                            Behavior on color { ColorAnimation { duration: 140 } }

                            // Animación de entrada escalonada
                            opacity: appLauncherOverlay.openProgress
                            
                            transform: [
                                Translate {
                                    y: (1 - appLauncherOverlay.openProgress) * (12 + index * 3)
                                },
                                Scale {
                                    origin.x: width / 2; origin.y: height / 2
                                    xScale: appMa.containsMouse ? 1.05 : 1.0
                                    yScale: appMa.containsMouse ? 1.05 : 1.0
                                    Behavior on xScale { NumberAnimation { duration: 120 } }
                                    Behavior on yScale { NumberAnimation { duration: 120 } }
                                }
                            ]
                            ...
                            ColumnLayout {
                                ...
                                Text {
                                    ...
                                    text: {
                                        ...
                                        if (n.indexOf("brave") >= 0) return "\uf268";
                                        if (n.indexOf("chrom") >= 0) return "\uf268";
                                        if (n.indexOf("code") >= 0 || n.indexOf("codium") >= 0) return "\ue70c";
                                        if (n.indexOf("kitty") >= 0 || n.indexOf("terminal") >= 0 || n.indexOf("alacritty") >= 0) return "\uf120";
                                        if (n.indexOf("thunar") >= 0 || n.indexOf("nautilus") >= 0 || n.indexOf("files") >= 0 || n.indexOf("archivos") >= 0) return "\uf07c";
                                        if (n.indexOf("spotify") >= 0) return "\uf1bc";
                                        if (n.indexOf("discord") >= 0) return "\uf392";
                                        if (n.indexOf("telegram") >= 0) return "\uf2c6";
                                        if (n.indexOf("steam") >= 0) return "\uf1b6";
                                        if (n.indexOf("obs") >= 0) return "\uf03d";
                                        if (n.indexOf("vlc") >= 0 || n.indexOf("mpv") >= 0) return "\uf03d";
                                        if (n.indexOf("gimp") >= 0) return "\uf1fc";
                                        if (n.indexOf("blender") >= 0) return "\uf1b2";
                                        if (n.indexOf("settings") >= 0 || n.indexOf("config") >= 0) return "\uf013";
                                        return "\uf2d0";
                                    }
                                    color: appMa.containsMouse ? theme.accent : theme.text
                                    font.family: "Symbols Nerd Font"
                                    font.pixelSize: 24
                                    Behavior on color { ColorAnimation { duration: 140 } }
                                }
                            ...
                            Rectangle {
                                ...
                                Text {
                                    anchors.centerIn: parent
                                    text: favoritesManager.isFavorite(modelData) ? "\uf005" : "\uf006"
                                    color: favoritesManager.isFavorite(modelData) ? theme.accent : theme.textDim
                                    font.family: "Symbols Nerd Font"
                                    font.pixelSize: 10
                                    layer.enabled: favoritesManager.isFavorite(modelData)
                                    layer.effect: MultiEffect { shadowEnabled: true; shadowColor: theme.accent; shadowBlur: 0.5 }
                                ...
                                MouseArea {
                                    ...
                                    cursorShape: Qt.PointingHandCursor
                                    onClicked: {
                                        modelData.icon = appIconTxt.text;
                                        favoritesManager.toggle(modelData);
                                    }
                                }
                            }
                ...
                // Estado vacío
                ColumnLayout {
                    visible: appLauncherOverlay.filtered.length === 0 && appLauncherOverlay.query.length > 1
                    Layout.alignment: Qt.AlignHCenter
                    Layout.topMargin: 32
